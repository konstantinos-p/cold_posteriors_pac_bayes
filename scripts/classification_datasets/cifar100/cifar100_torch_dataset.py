...
'''
In this module I include all functions necessary to use the CIFAR-100 dataset.
'''

def CIFAR100_split(dir='dataset',mode='classification',validation_percentage=10,seed_split=42,image_transforms=False):
    '''
     A function returning a CIFAR100 dataset split after some transformations.

     Parameters
     ----------
     dir: The directory where the CIFAR100 raw data are located.
    ...
    if 'cifar100' not in os.listdir(dir):
        raise FileNotFoundError('The cifar100 folder doesn\'t exist in directory: '+dir )
    ...
    #Create the test set
    test = CIFAR100(root=dir+'/cifar100', download=True, train=False, transform=default_transform, target_transform=target_transform)

    #Create a temporary train set
    train_tmp = CIFAR100(root=dir+'/cifar100', download=True, train=True, transform=image_transform, target_transform=target_transform)
...
def get_dataloaders(validation_percentage=10,seed_split=42,batch_size=100,dir=None,mode='classification',image_transforms=False):
    """
    Return dataloaders for all splits of CIFAR100.
    """

    test, train, validation = \
        CIFAR100_split(dir=dir, mode=mode, validation_percentage=validation_percentage, seed_split=seed_split,image_transforms=image_transforms)
