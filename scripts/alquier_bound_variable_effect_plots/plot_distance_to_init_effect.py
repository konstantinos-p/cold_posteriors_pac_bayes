...
when we change the values of the squared l2 difference between the prior and posterior weights.
...
#Plot Non Vacuous
count = 0
for prior_posterior_distance in [0.1,1,5]:
...
patch0 = Line2D([0], [0], marker='s', color='w', label=r'$||w_{\hat{\rho}}-w_{\pi}||_2^2=0.1$',
       markerfacecolor=colors[0], markersize=15)
patch1 = Line2D([0], [0], marker='s', color='w', label=r'$||w_{\hat{\rho}}-w_{\pi}||_2^2=1$',
       markerfacecolor=colors[1], markersize=15)
patch2 = Line2D([0], [0], marker='s', color='w', label=r'$||w_{\hat{\rho}}-w_{\pi}||_2^2=5$',
