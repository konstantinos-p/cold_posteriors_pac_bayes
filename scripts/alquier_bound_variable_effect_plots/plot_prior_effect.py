...
when we change the values of the prior "sigma_{\pi}^2".
...
#Plot Non Vacuous
count = 0
for prior_variance in [0.1,0.03,0.01]:
    lambda_bound = np.linspace(0, 1 / (2 * prior_variance * x_variance), num=100)
...
patch0 = Line2D([0], [0], marker='s', color='w', label=r'$\sigma_{\pi}^2=0.1$',
       markerfacecolor=colors[0], markersize=15)
patch1 = Line2D([0], [0], marker='s', color='w', label=r'$\sigma_{\pi}^2=0.03$',
       markerfacecolor=colors[1], markersize=15)
patch2 = Line2D([0], [0], marker='s', color='w', label=r'$\sigma_{\pi}^2=0.01$',
