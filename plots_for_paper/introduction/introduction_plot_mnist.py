...
"""
Plot for MNIST-10 for the introduction of the paper (Figure 1).
"""
def scale01(x):
    return x/np.max(x)
...
for i in range(10):
    results_file1 = open(path+'model_'+str(i)+"/results_0.pkl", "rb")
    output1 = pickle.load(results_file1)
    nlls.append(np.reshape(output1['nll'][1],(1,-1)))
    eces.append(np.reshape(output1['ECE'][1],(1,-1)))
