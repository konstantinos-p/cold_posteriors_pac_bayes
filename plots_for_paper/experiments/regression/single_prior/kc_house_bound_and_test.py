...
"""
Plot for the KC_House dataset original bounds for Figure 3 of the paper.
"""
def scale01(x):
    return x/np.max(x)
...
for prior,color_num in zip(priors,np.arange(len(priors))):
    ax1.plot(lambdas, offset3 + nlls_means[str(prior)], linewidth=5, c=cmap(0), linestyle='-')
...
plt.title('KC_HOUSE',fontsize=size_font_title)
ax1.vlines(1, 0.925,
           0.97, linewidth=5, linestyle='--',
           zorder=-1, alpha=0.5)
...
plt.savefig('kc_house_alquier.png',dpi=300)
