...
"""
Plot for the MNIST-10 dataset original bound for Figure 2 of the Appendix.
"""
def scale01(x):
    return x/np.max(x)
...
for i in range(10):
    for prior in  priors:
        ...
        nlls[str(prior)].append(np.reshape(output1['nll'][1],(1,-1)))
...
for prior,color_num in zip(priors,np.arange(len(priors))):
    plt.plot(lambdas,offset3+original_bounds_means[str(prior)],linewidth=5, c=colors[color_num],linestyle='--')
