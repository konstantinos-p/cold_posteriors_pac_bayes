...
"""
Plot for the MNIST-10 dataset zero-one loss for Figure 2 of the Appendix.
"""
def scale01(x):
    return x/np.max(x)
...
path = '/Users/Kostas/PycharmProjects/cold-warm-posteriors/cold_warm_posterior_experiments/' \
       'standard_data/classification_experiments/MNIST/results/MNIST_lambda_0_to_20/'
...
for i in range(10):
    for prior in  priors:
        ...
        nlls[str(prior)].append(np.reshape(output1['zero_one'][1],(1,-1)))
...
patches = []
for prior,color_num,marker in zip(priors,np.arange(len(priors)),['s','o','v']):
    plt.plot(lambdas,offset1+nlls_means[str(prior)],linewidth=5, c=colors[color_num],marker=marker)

    ax1.fill_between(lambdas, offset1+(nlls_means[str(prior)]-std_scale*nlls_stds[str(prior)]),
                     offset1+(nlls_means[str(prior)]+std_scale*nlls_stds[str(prior)]), color='grey', alpha=.5)

    patches.append(Line2D([0], [0], marker=marker, color='w', label='$\sigma^2_{\pi}='+str(prior_variance[prior])+'$',
