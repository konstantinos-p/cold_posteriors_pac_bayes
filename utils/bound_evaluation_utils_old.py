...
class bound_estimator:
    """
    This class evaluates the B_approximate,B_mixed and B_original bounds for a given model.
    ...
    def __init__(self,model,loss_fn,true_dataloader,train_dataloader,prior_variance=0.1,dataset_aleatoric=1,
        ...
        #To be set when computing the moment terms
        self.bound_terms = {}
    ...
    def fit(self):
        ...
        self.la = Laplace(self.model, likelihood=self.likelihood, prior_precision=1/self.prior_variance, subset_of_weights='all',
        ...
        if self.likelihood == 'regression':
            ...
            la_true = Laplace(self.model, likelihood='regression', prior_precision=1 / self.prior_variance, subset_of_weights='all',
    ...
    def estimate(self,bound_types={'original'}
                     ...
                     ,max_temperature=10,grid_lambda=100,prior_mean=None):
        """
        Computes the bounds.
        """
        ...
        if 'approximate' in  bound_types:
            self.lambdas = torch.linspace(self.min_temperature_reduced, self.max_temperature_reduced, grid_lambda).to(self.device)
            self.lambdas = self.lambdas[1:-1]
        else:
            self.lambdas = torch.linspace(min_temperature, max_temperature, grid_lambda).to(self.device)


        #Compute moment terms for the different bounds.
        self.compute_terms()
    ...
    def empirical_risk(self):
        ...
        '''
        return self.map_emp_risk+(1/(2*self.lambdas.cpu().numpy()*self.h.cpu().numpy()/self.n+1/self.prior_variance))*(self.h.cpu().numpy()/self.n)
    ...
    def moment(self):
        ...
        The moment term.
        '''
        return self.sigma_x.cpu().numpy()*(self.prior_variance*self.d+self.norm_generator.cpu().numpy())/(1-2*self.lambdas.cpu().numpy()*self.n*self.sigma_x.cpu().numpy()*self.prior_variance)\
    ...
    def kl(self):
        ...
        The KL term.
        '''
        a = (self.d/self.prior_variance)*(1/(2*self.lambdas.cpu().numpy()*self.h.cpu().numpy()/self.n+1/self.prior_variance))
        b = (1/self.prior_variance)*self.norm_diff.cpu().numpy()
        c = -self.d-self.d*np.log((1/(2*self.lambdas.cpu().numpy()*self.h.cpu().numpy()/self.n+1/self.prior_variance)))
    ...
    def moment_MC(self):
        ...
        res = (1/(self.n*np.reshape(lambdas.cpu().numpy(),(-1))))*(np.log(1/self.multipliers.shape[1])
                                                        +torch.logsumexp(self.n*lambdas@self.multipliers,dim=1).cpu().numpy())
    ...
    def empirical_risk_MC(self):
        ...
        for lambda_t in self.lambdas:
            ...
            risks.append(risk_evaluation_loop(dataloader=self.train_dataloader, la_model=self.la, loss_fn=self.loss_fn, model_averaging=False
